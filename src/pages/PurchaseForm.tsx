  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        const purchaseRef = editingId
          ? doc(db, "purchases", editingId)
          : doc(collection(db, "purchases"));

        const isM1Installment =
          paymentMethod === "cartão" || paymentMethod === "transferência";
        const isM2Installment =
          isSplitPayment &&
        ...
        // Part 1
        if (isM1Pending) {
          const amount1 = isSplitPayment ? (splitAmount1 - discount + interest) : finalTotal;
        ...
        // Part 2
        if (isM2Pending) {
          const amount2 = Math.max(0, (cartTotal + fee + freight - splitAmount1) - discount2 + interest2);
        ...
        const purchaseData = {
          ...
          splitAmount2: isSplitPayment
            ? Math.max(0, (cartTotal + fee + freight - splitAmount1) - discount2 + interest2)
            : 0,
          ...
          timestamp: format(
            toZonedTime(new Date(purchaseDate), "America/Sao_Paulo"),
        ...
        // 1.1 Record Financial Movement
        if (
          (!editingId ||
            originalPurchase.paymentStatus !== "paid" ||
            (isSplitPayment && originalPurchase.paymentStatus2 !== "paid"))
        ) {
          if (isSplitPayment) {
            const amount1 = splitAmount1 - discount + interest;
            const amount2 = Math.max(0, (cartTotal + fee + freight - splitAmount1) - discount2 + interest2);

            // First payment movement
            if (amount1 > 0 && paymentStatus === "paid" && (!editingId || originalPurchase.paymentStatus !== "paid")) {
              const movementRef1 = doc(collection(db, "cash_movements"));
              transaction.set(movementRef1, {
                amount: amount1,
                type: "out",
                category: "compra",
                paymentMethod: paymentMethod,
                reason: `Compra #${purchaseRef.id.slice(-6).toUpperCase()} (Parte 1/2)`,
                userId: profile.uid,
                userName: profile.name,
            ...
            // Second payment movement
            if (amount2 > 0 && paymentStatus2 === "paid" && (!editingId || originalPurchase.paymentStatus2 !== "paid")) {
              const movementRef2 = doc(collection(db, "cash_movements"));
              transaction.set(movementRef2, {
                amount: amount2,
                type: "out",
                category: "compra",
                paymentMethod: paymentMethod2,
                reason: `Compra #${purchaseRef.id.slice(-6).toUpperCase()} (Parte 2/2)`,
                userId: profile.uid,
                userName: profile.name,
                purchaseId: purchaseRef.id,
                timestamp: getBrasiliaISO(),
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold tracking-widest uppercase mt-1">
              {editingId
                ? `Nota #${editingId.slice(-6).toUpperCase()}`
                : "Registrar Entrada"}
            </p>
          </div>
        </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {sortSizes(
                            ...
                            <span
                              key={size}
                              className="px-1.5 py-0.5 bg-slate-100 text-[8px] font-bold text-slate-500 rounded uppercase"
                            >
                              {size}
                            </span>
                          ))}
                          ...
                          {product.sizes && product.sizes.length > 0 && (
                            <div className="flex flex-wrap justify-end gap-1 max-w-[150px] mt-1">
                              {sortSizes(product.sizes).map((size) => {
                                const itemSku = getProductSku(
                                  product.sku,
                                  size,
                                );
                                const isMatched = itemSku
                                  .toLowerCase()
                                  .includes(searchTerm.toLowerCase());
                                return (
                                  <button
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {sortSizes(
                      categories
                        .filter((c) => c.type === "tamanho")
                        .map((c) => c.name),
                    ).map((size) => {
                      const entry = sizeEntries[size] || {
                        qty: 0,
                        price: sizeModalProduct.costPrice || 0,
                      };
                      const isProductSize =
                        sizeModalProduct.sizes?.includes(size) ||
                        sizeModalProduct.size === size;
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-4">
                <div className="text-left">
                  ...
                  <p className="text-lg font-black text-primary">
                    {Object.values(sizeEntries).reduce(
                      (acc: number, curr: any) => acc + (curr.qty || 0),
                      0,
                    )}{" "}
                    <span className="text-xs font-bold">unidades</span>
                  </p>
                </div>
                <button
