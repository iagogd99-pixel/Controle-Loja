...
export default function Clients() {
  ...
  const openForm = (client?: Client, e?: React.MouseEvent) => {
    e?.stopPropagation();
    if (client) {
    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 px-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-primary tracking-tight leading-none">Clientes</h1>
            <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mt-1">Base de Contatos e Fidelidade</p>
          </div>
        </div>
        <button 
          onClick={() => openForm()}
        ...
        {filteredClients.map((client) => (
          ...
          >
            <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center mb-3 group-hover:bg-accent/10 transition-colors">
              <Users className="w-5 h-5 text-slate-400 group-hover:text-accent" />
            </div>
      ...
      {filteredClients.length === 0 && !loading && (
        <div className="py-20 text-center px-4">
          <div className="bg-white p-8 rounded-[40px] border border-dashed border-slate-200 inline-block w-full max-w-sm">
            ...
            <p className="text-[10px] text-slate-300 font-black tracking-widest uppercase">Nenhum cliente cadastrado</p>
          </div>
        </div>
      )}
      ...
      <AnimatePresence>
        {selectedClient && !isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            />
            <motion.div 
              layoutId={`client-${selectedClient.id}`}
            ...
            >
              ...
              <div className="p-8 pb-6 bg-accent text-white">
                ...
                <p className="text-[10px] font-bold opacity-60 uppercase tracking-widest mt-1">Perfil do Cliente</p>
              </div>

              <div className="flex-1 overflow-y-auto p-8 space-y-4">
                <InfoItem icon={Mail} label="E-mail" value={selectedClient.email || 'Não informado'} />
                <InfoItem icon={Phone} label="WhatsApp" value={selectedClient.phone || 'Não informado'} />
                <InfoItem icon={CreditCard} label="Documento (CPF)" value={selectedClient.cpf || 'Não informado'} />
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
             ...
             >
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  <FormInput label="Nome Completo" value={formData.name} onChange={(v) => setFormData({...formData, name: v})} />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <FormInput label="CPF" value={formData.cpf} onChange={(v) => setFormData({...formData, cpf: v})} />
                    <FormInput label="Nascimento" type="date" value={formData.birthDate} onChange={(v) => setFormData({...formData, birthDate: v})} />
                  </div>

                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Endereço Completo</label>
                    <textarea 
                      rows={2}
